...
6. Nothing billable is undocumented: teardown exists and covers what setup creates.
7. last_verified is not stale.
...
# linking it. Keeps three repos from drifting the way PuppyGraph's demos did.
INLINE_CONNECT_RE = re.compile(
    r"(upload\s+(the\s+)?(service\s+)?account\s+key|select\s+region.*select\s+graph)",
    re.I | re.S,
)
...
def _stringify_dates(obj):
    """PyYAML turns an unquoted `2026-08-13` into a datetime.date, which then fails a
...
def main() -> int:
    ...
    for d in demo_dirs:
        ...
        if manifest.exists():
            ...
            # Anything billable must be bounded and removable.
            cost = data.get("cost") or {}
            if cost.get("billable_resources"):
                if not (d / "scripts/teardown.sh").exists():
                    err(where, "creates billable resources but has no teardown.sh")
                creates = [c for s in data.get("steps", []) for c in (s.get("creates") or [])]
                if not creates:
                    err(where, "creates billable resources but no step declares `creates:`")

            lv = data.get("last_verified")
            if isinstance(lv, dt.date):
                age = (today - lv).days
                if age > STALE_DAYS:
                    warn(where, f"last_verified is {age} days old — re-run it or mark it preview")
